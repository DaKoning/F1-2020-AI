...
# data: speed, ray_front, ray_right, ray_left, ray_rightfront, ray_leftfront, totalDistance, totalDistance_old, currentLapInvalid, throttle, brakes, steering
...
data = np.array([0, 0, 0, 0, 0, 0, 0, startDistance, 0, 0, 0, 0], dtype=object)
...
def run(data):
    ...
    while True:
        ...
        if isinstance(packet, f1_2020_telemetry.packets.PacketLapData_V1):
            ...
            throttle = round(packet.carTelemetryData[0].throttle * 10)
            brakes = round(packet.carTelemetryData[0].brake * 10)
            steering = round(packet.carTelemetryData[0].steer * 10)
            data[9] = throttle
            data[10] = brakes
            data[11] = steering
